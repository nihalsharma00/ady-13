...
const Bollywood = () => {
    ...
    useEffect(() => {
        document.title = `Blog Machine - Bollywood`
    })

    const [topthree] = useState(context.sort(() => Math.random() - Math.random()).filter(item => item.category === 'Bollywood').slice(0, 3))

    const [topContext] = useState(context.sort(() => Math.random() - Math.random()).filter(item => item.category === 'Bollywood').slice(0, 10))

    const [letfTop] = useState(context.sort(() => Math.random() - Math.random()).filter(item => item.category === 'Bollywood').slice(0, 5))
